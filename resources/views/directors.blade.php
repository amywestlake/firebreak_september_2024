...
<body class="bg-gray-100 text-gray-800">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <section class="mb-8">
            <h2 class="text-2xl font-semibold mb-4">Directors</h2>
            <div class="table_component" role="region" tabindex="0" class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-300"id="myTable">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="py-3 px-6 text-left text-gray-700 font-semibold uppercase text-sm border-b">Director</th>
                    ...
                    @foreach($directorsWithFilmCountOrderByTotalFilms as $director)
                    <tr class="hover:bg-gray-50">
                        <td class="py-4 px-6 border-b">{{$director->name}}</td>
                        <td class="py-4 px-6 border-b"><strong>{{$director->total_films}}</strong>: {{$director->film_titles}}</td>
                        <td class="py-4 px-6 border-b">{{ number_format($director->critic_votes / $totalCriticVotes * 100, 2) }}%<br/>({{$director->critic_votes}})</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
